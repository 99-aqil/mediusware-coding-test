...
const CreateProduct = (props) => {
    ...
    let saveProduct = (event) => {
        ...
        const productSkuInput = document.querySelector('input[name="product_sku"]');
        const descriptionInput = document.querySelector('textarea[name="description"]');
        if (productNameInput && productSkuInput && descriptionInput) {
          formData.append('product_name', productNameInput.value);
          formData.append('product_sku', productSkuInput.value);
          formData.append('description', descriptionInput.value);
        }
      
        // Add the product variants data to the form data
        const productVariantsData = [];
        document.querySelectorAll('.product-variant').forEach((productVariant) => {
          const optionSelect = productVariant.querySelector('select[name="option"]');
          const tagsInput = productVariant.querySelector('input[name="tags"]');
          if (optionSelect && tagsInput) {
            const option = optionSelect.value;
            const tags = tagsInput.value.split(',');
            productVariantsData.push({ option, tags });
          }
        });
        formData.append('product_variants', JSON.stringify(productVariantsData));
      
        // Add the product variant prices data to the form data
        const productVariantPricesData = [];
        document.querySelectorAll('.product-variant-price').forEach((productVariantPrice) => {
          const titleElement = productVariantPrice.querySelector('td:first-child');
          const priceInput = productVariantPrice.querySelector('input[name="price"]');
          const stockInput = productVariantPrice.querySelector('input[name="stock"]');
          if (titleElement && priceInput && stockInput) {
            const title = titleElement.textContent;
            const price = priceInput.value;
            const stock = stockInput.value;
            productVariantPricesData.push({ title, price, stock });
          }
        });
        formData.append('product_variant_prices', JSON.stringify(productVariantPricesData));
      
        // Make the POST request to the Django REST API
        fetch('/product', {
          method: 'POST',
          body: formData,
          headers: {
